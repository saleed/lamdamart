	...
	def fit(self):
		...
		for k in range(self.number_of_trees):
			...
			for lambda_val, w_val, query_key in pool.map(compute_lambda, zip(true_scores, pred_scores, good_ij_pairs, idcg, query_keys), chunksize=1):
				...
				w[indexes] = w_val
			pool.close()
			# test=zip(true_scores, pred_scores, good_ij_pairs, idcg, query_keys)
			# print(test)
			# lamd=compute_lambda(test)
			# for lambda_val, w_val, query_key in lamd:
			# 	indexes = query_indexes[query_key]
			# 	lambdas[indexes] = lambda_val
			# 	w[indexes] = w_val
			...
			if self.tree_type == 'sklearn':
				...
				if len(self.trees)<k+1:
					self.trees.append(DecisionTreeRegressor(max_depth=50))
				tree= self.trees[k]
